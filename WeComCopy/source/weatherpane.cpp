    ...
    if (!doucment.isNull() && (jsonError.error == QJsonParseError::NoError))
    {
        if (doucment.isObject())
        {
            ...
            if (object.contains("status"))
            {
                ...
                if (nStatus != 1000) //status1000 表示请求成功
                    return;
            }
...
{
    if (TABTITLE_WEDRIVE == eMainTabTitle)
        UpdateWeather();
}
...
void CWeatherPane::UpdateWeather()
{
    ...
    TMsgItem item;
    item.strUrl = QString("http://wthrcdn.etouch.cn/weather_mini?city=长沙");
    item.pObj = this;
    MSGQUEUE->Push(item);
}
...
int CWeatherPane::DrawWeather(QPainter *painter)
{
    ...
    painter->drawText(rectCity, Qt::AlignTop | Qt::AlignLeft, "长沙");
    painter->setFont(ft);
