...
{
    if (obj == m_labUserName)   //用户名标签
    {
        if (event->type() == QEvent::MouseButtonPress)  //单击用户名时 显示用户详细信息窗口
        {
            m_pUserDetail->SetUserInfo(m_labUserName->text(), m_tUserInfo.strAvatar, m_tUserInfo.strEmail, m_tUserInfo.strPart);
            m_pUserDetail->show();
