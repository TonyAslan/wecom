...
void TcpServer::incomingConnection(qintptr socketDescriptor)
{
	...
	connect(tcpsocket, SIGNAL(signalGetDataFromClient(QByteArray&, int)),
		this, SLOT(onSocketDataProcessing(QByteArray&, int)));
	//收到客户端断开连接后，server进行处理
	connect(tcpsocket, SIGNAL(signalClientDisconnect(int)),
		this, SLOT(onSocketDisconnected(int)));
...
void TcpServer::onSocketDisconnected(int descriptor)
{
	for (int i = 0; i < m_tcpSocketConnectList.count(); ++i)
	{
		...
		{
			emit signalSocketDisconnected(item);
...
void TcpServer::onSocketDataProcessing(QByteArray& sendData, int descriptor)
{
	for (int i = 0; i < m_tcpSocketConnectList.count(); ++i)
	{
		...
		if (item->socketDescriptor() == descriptor)
		{
			...
			emit signalTcpMsgComes(item, sendData);
		}
	}
}
